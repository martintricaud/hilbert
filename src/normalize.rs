...
impl IntegerDataRange {
        ...
        I: Into<i64>,
    {
        ...
        if bits_allocated < self.bits_required {
            uncompressed >> (self.bits_required - bits_allocated)
        } else {
            uncompressed
        }
    }
...
mod tests {
    ...
    use super::{FloatDataRange, IntegerDataRange};
    ...
    fn compress_i32() {
        ...
        assert_eq!(
            actual_compressed, expected_compressed,
            "Compressed value correct"
        );
    }

    #[test]
    fn smallest_power_of_two() {
        let in_out_pairs = vec![
            (0_u64, 0),
            (1, 0),
            (2, 1),
            (3, 2),
    ...
    fn test_points_i32() -> Vec<Vec<i32>> {
        vec![vec![-10, 5, 3], vec![-4, 20, 100], vec![42, -100, 0]]
    }
